...
import { updateQuoteLines } from '@/actions/quotes';
...
function newLine(): Line {
  nextId += 1;
  return { id: `new-${nextId}`, description: '', quantity: 1, cost: 0, expensePct: DEFAULT_EXPENSE_PCT, marginPct: DEFAULT_MARGIN_PCT, unitPrice: 0 };
}
    ...
    return (
      <div className="overflow-hidden rounded-2xl border border-line bg-white">
        ...
        <div className="border-t border-line px-4 py-3 text-right text-sm font-medium text-ink">
          Total: {currency} {initial.reduce((sum, l) => sum + l.lineTotal, 0).toFixed(2)}
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="rounded-2xl border border-line bg-white p-4">
      ...
      <div className="space-y-2">
        {lines.map((l) => (
          <div key={l.id} className="grid grid-cols-2 gap-2 sm:grid-cols-7">
            <input
              value={l.description}
            ...
            <input
              value={l.expensePct}
              onChange={(e) => updateLine(l.id, { expensePct: Number(e.target.value) })}
              type="number"
              placeholder="Exp %"
              className="rounded-lg border border-line px-3 py-2 text-sm text-ink"
            />
            <input
              value={l.marginPct}
              onChange={(e) => updateLine(l.id, { marginPct: Number(e.target.value) })}
              type="number"
              placeholder="Margin %"
              className="rounded-lg border border-line px-3 py-2 text-sm text-ink"
            />
